...
namespace App\Http\Controllers\Admin\Product\Krafts;
...
class KraftsController extends Controller
{
    public function showProductForm () 
    {
        ...
        return view('admin.product.krafts.new_product', ['top_category' => $top_category, 'data' => $all_sub_category]);
    }
    ...
    {
        return view('admin.product.krafts.product_list.product_list');
    }
    ...
    public function productListData(Request $request)
    {
        ...
        if(empty($request->input('search.value'))) {            
            ...
            $product_data = DB::table('product')
                            ...
                            ->where('top_category.id', 4)
                            ->select('product.*', 'sub_category.sub_cate_name', 'top_category.top_cate_name', 'third_level_sub_category.third_level_sub_category_name')
                            ->offset($start)
                            ->limit($limit)
                            ->orderBy($order,$dir)
                            ->get();
        ...
        if(!empty($product_data)) {
            ...
            foreach ($product_data as $single_data) {
                ...
                if($single_data->status == 1)
                    $val = "<a href=\"".route('admin.change_krafts_product_status', ['product_id' => encrypt($single_data->id), 'status' => encrypt(0)])."\" class=\"btn btn-primary\">In-Active</a>";
                else
                    $val = "<a href=\"".route('admin.change_krafts_product_status', ['product_id' => encrypt($single_data->id), 'status' => encrypt(1)])."\" class=\"btn btn-primary\">Active</a>";
                ...
                $nestedData['product_banner']  = "&emsp;<a href=\"".route('admin.edit_krafts_product_banner', ['product_id' => encrypt($single_data->id)])."\" class=\"btn btn-success\" target=\"_blank\">Change Banner</a>";

                $nestedData['product_images']  = "&emsp;<a href=\"".route('admin.additional_krafts_product_image_list', ['product_id' => encrypt($single_data->id)])."\" class=\"btn btn-primary\" target=\"_blank\">Product Images List</a>";

                $nestedData['action']  = "&emsp;<a href=\"".route('admin.view_krafts_product', ['product_id' => encrypt($single_data->id)])."\" class=\"btn btn-success\" target=\"_blank\">View</a>&emsp;<a href=\"".route('admin.edit_krafts_product', ['product_id' => encrypt($single_data->id)])."\" class=\"btn btn-warning\" target=\"_blank\">Edit</a>&emsp;$val";
    ...
    public function showEditProductBanner($product_id)
    {
        ...
        return view('admin.product.Krafts.banner.edit_banner' , ['url' => $url, 'product_id' => $product_id]);
    }
    ...
    public function showProductImageList($product_id)
    {
        ...
        return view('admin.product.krafts.additional_image.additional_image' , ['additional_image_record' => $additional_image_record, 'product_record' => $product_record]);
    }
    ...
    public function viewProduct($product_id)
    {
        ...
        return view('admin.product.krafts.action.view_product', ['product_record' => $product_record]);
    }
    ...
    public function showEditProduct ($product_id) 
    {
        ...
        return view('admin.product.krafts.action.edit_product', ['all_top_category' => $all_top_category, 'all_sub_category' => $all_sub_category, 'product_record' => $product_record, 'all_third_level_sub_category' => $all_third_level_sub_category]);
    }
