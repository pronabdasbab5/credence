      ...
      <div class="col-md-12 col-sm-12 col-xs-12">
        <div class="x_panel">
          ...
          <div class="x_content"><br />
            ...
            <form method="POST" autocomplete="off" action="{{ route('admin.update_apparel_product_color', ['product_id' => encrypt($product_record[0]->id)]) }}" class="form-horizontal form-label-left">
                @csrf
                        ...
                        @if (count($product_colors) > 0)
                            ...
                            @foreach ($product_colors as $key => $item)
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if($item->status == 1)
                                            <a href="{{ route('admin.change_apparel_product_color_status', ['product_mapping_id' => encrypt($item->id), 'status' => encrypt(0)]) }}" class="btn btn-warning form-text-element">In-Active</a>
                                        @else
                                            <a href="{{ route('admin.change_apparel_product_color_status', ['product_mapping_id' => encrypt($item->id), 'status' => encrypt(1)]) }}" class="btn btn-warning form-text-element">Active</a>
                                        @endif
                                    </td>
                                </tr> 
                            @endforeach
